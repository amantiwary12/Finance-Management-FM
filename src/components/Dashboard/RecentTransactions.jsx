            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-1">
                  <span className="text-xs font-medium text-gray-800">{transaction.category}</span>
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${getTypeStyles(transaction.type)}`}>
                    {transaction.type}
                  </span>
                </div>
                <p className="text-xs text-gray-500">{formatDate(transaction.date || transaction.createdAt)}</p>
                {transaction.note && (
